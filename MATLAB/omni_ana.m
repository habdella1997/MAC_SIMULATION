...
%math.ceil(control_packet_length / MAC_Controller.FEC_NUMERATOR) * MAC_Controller.FEC_DENOMINATOR
control_packet_size = ceil((24*8)/15)*16;
data_packet_size    = ceil((64000 * 8)/15) * 16;

d = 5:1:40; % Considering NLoS Scenerio
...
for k = 1:length(inter_arrival_time_list)
    ...
    for i = 1:10
        ...
        %T_c = (i) * ((tpropmean*2));
        T_c         = (i) * ((tpropmean*2) + mean_backoff + T_rts + T_cts + ((2^i)*1e-9)) ;
    ...
    t_prop2 = 4* tpropmean; %% RTS, CTS, UL, ACK
    t_data2 = data_packet_size/T_data;
    arg     =  t_prop2 + T_cts + T_rts + t_data2 + T_ack;
    idx2 = findClosestIndex(t , arg);
    f_T_sector(1:idx2) = ones(1,length(f_T_sector(1:idx2))).*1 ;
