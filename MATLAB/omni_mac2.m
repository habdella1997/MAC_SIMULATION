...
control_packet_size = ceil((24*8)/15)*16;
data_packet_size    = ceil((64000 * 8)/15) * 16;
d = 15:1:65;
...
inter_arrival_time_list = (100:50:1000) .* 1e-6 ; 
S_Results = [];
t = 1e-9:1e-9:300e-6;
Nnodes = 50;
for k = 1:length(inter_arrival_time_list)
    ...
    lambda = 1/tia;
    mu     = 1/T_data;
    rho = (Nnodes*lambda)/mu;

    mean_backoff = T_bo_max;
    
    p_collisions = 1; p_collision_poisson = 1 - (exp(-1*(lambda)*2*T_rts));
    %% Compute T_rts
    for i = 1:10
        p_collision_poisson = p_collision_poisson*p_collision_poisson;
        T_c         = (i) * ((tpropmean*2) + mean_backoff + 2*T_rts + ((2^i)*1e-9)) ;
        idx = findClosestIndex(t , T_c);
        f_T_rts(1:idx) = f_T_rts(1:idx) + (ones(1,length(f_T_rts(1:idx))).*p_collision_poisson);
    end
%     P_succ = 1-p_collision_poisson;
%     T_c         = 0;
%     idx = findClosestIndex(t , T_c);
%     f_T_rts(1:idx) = f_T_rts(1:idx) + (ones(1,length(f_T_rts(1:idx))).*P_succ);
